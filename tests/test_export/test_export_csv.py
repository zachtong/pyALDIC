...
def test_csv_has_disp_columns(tmp_path, minimal_result):
    paths = export_csv(tmp_path, "exp", "ts", minimal_result, fields=_ALL_DISP)
...
def test_csv_selective_fields(tmp_path, minimal_result):
    """Only the requested fields appear as columns."""
    paths = export_csv(tmp_path, "exp", "ts", minimal_result,
                       fields=["disp_u", "strain_exx"])
    with open(paths[0], encoding="utf-8-sig") as f:
        fieldnames = csv.DictReader(f).fieldnames or []
    assert "disp_u" in fieldnames
    assert "strain_exx" in fieldnames
    assert "disp_v" not in fieldnames
    assert "strain_eyy" not in fieldnames
