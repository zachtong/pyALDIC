...
% amssymb provides \blacktriangle and \blacktriangledown (dropdown /
% warning glyphs used in the Parameters and Strain sections).
\usepackage{amssymb}
...
% NOTE: the outer {...} around the spanning-header content is mandatory.
% Without it, the \\[2pt] inside is parsed as closing \twocolumn's own
% optional argument at the first ']', cascading into 'Extra }' errors.
\twocolumn[{%
\begin{center}
...
\begin{itemize}
  \item \textbf{From PyPI} (easiest): \texttt{pip install al-dic}.
  \item \textbf{From a GitHub Release wheel}: download
        \texttt{al\_dic-*.whl} from
        \href{https://github.com/zachtong/pyALDIC/releases}{github.com/zachtong/pyALDIC/releases}
        and run \texttt{pip install al\_dic-*.whl}. Useful when
        behind a firewall that blocks PyPI.
  \item \textbf{From source} (development):
        \texttt{git clone https://github.com/zachtong/pyALDIC.git}
        then \texttt{pip install -e ".[dev]"} inside the clone.
\end{itemize}

Requires Python $\geq 3.10$. Launch:
