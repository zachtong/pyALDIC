...
| File | Action | Purpose |
|------|--------|---------|
| `src/al_dic/mesh/refinement.py` | **Create** | `RefinementCriterion`, `RefinementContext`, `RefinementPolicy`, `refine_mesh()` |
| `src/al_dic/mesh/criteria/mask_boundary.py` | **Create** | `MaskBoundaryCriterion` — port of current `mark_edge` logic |
| `src/al_dic/mesh/criteria/manual_selection.py` | **Create** | `ManualSelectionCriterion` — user-specified element indices |
| `src/al_dic/mesh/criteria/posterior_error.py` | **Create** | `PosteriorErrorCriterion` — convergence/residual based |
| `src/al_dic/mesh/criteria/__init__.py` | **Create** | Re-exports |
| `src/al_dic/core/pipeline.py` | **Modify** | Add pre-solve and post-solve refinement hooks |
| `src/al_dic/core/data_structures.py` | **Modify** | (No change to DICPara — policy is a run_aldic param) |
| `src/al_dic/mesh/generate_mesh.py` | **Modify** | Extract `_post_refine_pipeline()` for reuse |
...
**Files:**
- Create: `src/al_dic/mesh/refinement.py`
...
from al_dic.core.data_structures import DICPara, DICMesh
...
```bash
cd al-dic && python -m pytest tests/test_mesh/test_refinement.py -v
```
Expected: FAIL — `ImportError: cannot import name 'RefinementContext' from 'al_dic.mesh.refinement'`

### Step 3: Implement the framework

```python
# src/al_dic/mesh/refinement.py
...
```bash
git add src/al_dic/mesh/refinement.py tests/test_mesh/test_refinement.py
...
**Files:**
- Modify: `src/al_dic/mesh/generate_mesh.py`
...
```bash
cd al-dic && python -c "from al_dic.mesh.generate_mesh import _reorder_element_nodes_ccw; print('OK')"
```
Expected: `OK`
...
**Files:**
- Create: `src/al_dic/mesh/criteria/__init__.py`
- Create: `src/al_dic/mesh/criteria/mask_boundary.py`
...
```bash
cd al-dic && python -m pytest tests/test_mesh/test_criteria_mask.py -v
```
Expected: FAIL — `ModuleNotFoundError: No module named 'al_dic.mesh.criteria'`

### Step 3: Implement

```python
# src/al_dic/mesh/criteria/__init__.py
...
```python
# src/al_dic/mesh/criteria/mask_boundary.py
...
```bash
git add src/al_dic/mesh/criteria/ tests/test_mesh/test_criteria_mask.py
...
**Files:**
- Modify: `src/al_dic/mesh/generate_mesh.py:45-132`
- Test: existing `tests/test_mesh/test_generate_mesh.py` (must still pass)

### Step 1: Run existing tests to confirm baseline

```bash
cd al-dic && python -m pytest tests/test_mesh/test_generate_mesh.py -v
```
Expected: All PASS
...
```bash
git add src/al_dic/mesh/generate_mesh.py
...
**Files:**
- Modify: `src/al_dic/core/pipeline.py:341-350` (run_aldic signature)
- Modify: `src/al_dic/core/pipeline.py:504-591` (Section 3)
...
```bash
cd al-dic && python -m pytest tests/test_integration/test_refinement_pipeline.py -v
```
...
```bash
cd al-dic && python -m pytest tests/ -v --timeout=120
```
...
```bash
git add src/al_dic/core/pipeline.py tests/test_integration/test_refinement_pipeline.py
...
**Files:**
- Create: `src/al_dic/mesh/criteria/manual_selection.py`
- Modify: `src/al_dic/mesh/criteria/__init__.py`
...
```python
# src/al_dic/mesh/criteria/manual_selection.py
...
```bash
git add src/al_dic/mesh/criteria/manual_selection.py src/al_dic/mesh/criteria/__init__.py tests/test_mesh/test_criteria_manual.py
...
**Files:**
- Create: `src/al_dic/mesh/criteria/posterior_error.py`
- Modify: `src/al_dic/mesh/criteria/__init__.py`
...
from al_dic.mesh.criteria.posterior_error import PosteriorErrorCriterion
from al_dic.mesh.refinement import RefinementContext, RefinementCriterion
from al_dic.mesh.mesh_setup import mesh_setup
from al_dic.core.data_structures import DICPara
...
```python
# src/al_dic/mesh/criteria/posterior_error.py
...
```bash
git add src/al_dic/mesh/criteria/posterior_error.py src/al_dic/mesh/criteria/__init__.py tests/test_mesh/test_criteria_posterior.py
...
**Files:**
- Modify: `src/al_dic/core/pipeline.py` (after Section 6, before cumulative)
- Modify: `src/al_dic/solver/local_icgn.py` (return conv_iterations)
...
```bash
cd al-dic && python -m pytest tests/test_integration/test_refinement_pipeline.py -v --timeout=120
```
...
**Files:**
- Modify: `src/al_dic/core/pipeline.py` (frame loop mesh reset logic)
- Test: `tests/test_integration/test_refinement_pipeline.py`
